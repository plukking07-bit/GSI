...
export default function EvaluatePage(): JSX.Element {
  ...
  const handleExplanationChange = (criteriaId: string, explanation: string): void => {
    setFormData((prev) => ({
      ...prev,
      explanations: {
        ...prev.explanations,
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] py-12 md:py-20 user-select-none">
                ...
                <div className="space-y-6">
                  {criterion.subCriteria.map((sub) => (
                    <div key={sub.id} className="border-t border-gray-200 pt-6 first:border-t-0 first:pt-0">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4 user-select-none">
                        {Array.from({ length: sub.maxScore + 1 }, (_, i) => i).map((score) => (
                          <button
                            key={score}
                            type="button"
                            onClick={() => handleScoreChange(sub.id, score)}
                            className={`flex-1 min-w-[50px] px-4 py-3 rounded-lg font-display font-semibold transition-all duration-200 pointer-events-auto cursor-pointer user-select-none ${
                      ...
                      <div className="mt-4">
                        <label className="block font-body text-sm font-medium text-gray-700 mb-2">
                          เพราะอะไรถึงเลือกคะแนนเท่านี้?
                        </label>
                        <textarea
                          value={formData.explanations[sub.id] || ''}
                          onChange={(e) => handleExplanationChange(sub.id, e.target.value)}
                          className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg font-body focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-colors hover:border-primary/50 resize-none"
                          placeholder="กรุณากรอกคำอธิบายว่าทำไมถึงเลือกคะแนนนี้..."
                          rows={1}
                        />
                      </div>
                ...
                {(() => {
                  ...
                  return guide ? (
                    <div className="mt-8 pt-6 border-t border-gray-200">
                      ...
                      >
                        <h4 className="font-display text-sm font-semibold text-cyan-700">
                          คู่มือการให้คะแนนเกณฑ์นี้
                        </h4>
                        <span className="font-display text-cyan-700 font-semibold group-hover:scale-110 transition-transform">
                          {isExpanded ? '▼' : 'ⓘ'}
                        </span>
                      </button>
                      ...
                      {isExpanded && (
                        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 animate-slide-up">
                          {guide.levels.map((level) => (
                            <div
                              key={level.score}
                              className="bg-gradient-to-br from-cyan-50 to-white rounded-lg p-3 shadow-sm border-l-4 border-cyan-400"
                            >
                              <div className="flex items-start space-x-2">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="font-body font-semibold text-gray-900 text-xs">
                                    {level.label}
                                  </p>
